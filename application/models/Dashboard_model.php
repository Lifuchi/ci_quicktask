...
class Dashboard_model extends CI_Model {
  ...
  public function __construct()
  {
	...
	public function getProgressBar(){
		$query = "SELECT te.T_ID, te.T_NAME ,te.T_USER, te.T_SINGKATAN, COALESCE(COUNT(IF(t.`TA_STATUS`= 100 ,1 , NULL))) AS done  ,COALESCE(COUNT(t.`TA_STATUS`)) AS alls
		FROM qt_team te
	...
	public function getNetworkOverall(){
		$query = "SELECT COALESCE(COUNT(IF(t.`TA_STATUS`= 100 ,1 , NULL))) AS done  ,COALESCE(COUNT(t.`TA_STATUS`)) AS alls
		FROM qt_team te
		LEFT JOIN qt_objective o ON te.T_ID = o.`T_ID`
		LEFT JOIN qt_task t ON o.`OBJECTIVE_ID` = t.`OBJECTIVE_ID`
		WHERE te.T_ID > 0";
